...
pdf("UMAP_clusters_with_annotation.pdf")
DimPlot(merged_seurat_integrated, reduction = "umap")
dev.off()

Idents(merged_seurat_integrated) <- "cluster_names"

pdf("UMAP_clusters_no_annotation.pdf")
DimPlot(merged_seurat_integrated, reduction = "umap", label = FALSE, pt.size = 0.5) + NoLegend()
dev.off()
## save plot
ggsave("UMAP_clusters_no_annotation.png" , device=png, dpi = 600, path = "/home/data/Testa_organoids_scRNA/Surat_code/outputs/figures", bg = "transparent", width = 25, height = 15, units = "cm" )
...
pdf("THRA1_THRA2_dotplot_day_50_100.pdf")
...
### new, generic names
## Assign names of all remaining clusters
new.simple.cluster.ids <- c("RG-1", "Proliferating RG", "oRG-1","RG - enriched ribosomes", "oRG-4", "oRG-5", "OPCs","RG-2", "APCs", "oRG-3","oRG-2", "Choroid Plexus cells", "NPCs-1", "Neurons - ribosome enriched","NPCs-2",  "Neurons-4",   "Neurons-2",    "Neurons-3", "Neurons-4" )
names(new.simple.cluster.ids) <- levels(merged_seurat_integrated)
Idents(merged_seurat_integrated) <- "cluster_names_final"
merged_seurat_integrated <- RenameIdents(merged_seurat_integrated, new.simple.cluster.ids)


merged_seurat_integrated <- RenameIdents(object = merged_seurat_integrated,  
                       'Glial cells' = 'NPCs-1',
                       'Immature Neurons' = 'Neurons-4',
                       "OPCs-1" = "oRG-4",
                       "OPCs-2" = "oRG-1",
                       "Neurons-1" = "Neurons-2",
                       "Excitatory Neurons" = "Neurons-1",
                       "RG" = "RG-1",
                       "RG - Astrocytes" = "RG-2",
                       "oRG" = "oRG-5",
                       "Neurons-2" = "Neurons-3",
                       ...
                       "NPCs-3" = "NPCs-2",
                       "Astrocytes" = "APCs",
                       "OPCs - Oligodendrocytes" = "OPCs")

merged_seurat_integrated$cluster_names_new <- as.character(Idents(merged_seurat_integrated))

## order the cell population in pseudotime
new_order <- c("RG-1",  "RG-2","RG - enriched ribosomes","Proliferating RG", "oRG-1", "oRG-2", "oRG-3","oRG-4", "oRG-5", "OPCs","APCs", "Choroid Plexus cells", "NPCs-1","NPCs-2", "Neurons - ribosome enriched","NPCs-3",  "Neurons-1",   "Neurons-2",    "Neurons-3", "Neurons-4" )
Idents(merged_seurat_integrated) <- factor(Idents(merged_seurat_integrated), levels= new_order)


table(Idents(merged_seurat_integrated))
...
DotPlot(merged_seurat_integrated, features = c("VIM","PTPRZ1", "FABP7","PTN", "HOPX", "PLP1", "APOE",  "TTR",  "STMN2", "DCX", "SMOC1", "IFITM3", "HES1", "TACC3", "UBE2C", "SOX3", "EOMES", "RPL26", "PDGFA",  "FGFR3", "PAX6" ), dot.scale = 6, dot.min = 0.24) + 
  theme(axis.title=element_blank()) + scale_colour_gradient(low =c("lightgrey"), high =c("red")) +
  scale_x_discrete(guide = guide_axis(angle = 75))

DotPlot(merged_seurat_integrated, features = c("VIM","RPL26", "SOX3", "UBE2C"), dot.scale = 6, dot.min = 0.24) + 
  theme(axis.title=element_blank()) + scale_colour_gradient(low =c("lightgrey"), high =c("red")) +
  scale_x_discrete(guide = guide_axis(angle = 75))

pdf("DotPlot_markers.pdf")
DotPlot(merged_seurat_integrated, features = c("VIM","RPL26", "UBE2C",  "FABP7", "PTN", "PLP1", "APOE", "TTR",  "STMN2","SYT1","DCX", "GAP43"), dot.scale = 6, dot.min = 0.24) + 
  theme(axis.title=element_blank()) + scale_colour_gradient(low =c("lightgrey"), high =c("red")) +
  scale_x_discrete(guide = guide_axis(angle = 75))
dev.off()
